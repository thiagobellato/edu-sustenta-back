...
from .serializers import (
    ...
    TeacherSchoolLinkSerializer # [NOVO]
)
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Normalizamos o role para evitar erros de maiúsculas
        ...
        if role == 'GESTOR':
            ...
            # [NOMES AJUSTADOS PARA O FRONT]
            data = {
                "total_escolas": minhas_escolas.count(),
                "total_professores": total_professores,
                "alunos_impactados": 0, # Placeholder
                "recentActivity": [
                    {
                        "id": link.id,
            ...
            # [CORREÇÃO CRÍTICA]: O Front espera 'escolas_vinculadas'
            data = {
...
# [NOVO] Permite que o PROFESSOR veja suas próprias escolas
class TeacherSchoolViewSet(ReadOnlyModelViewSet):
    serializer_class = TeacherSchoolLinkSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        # Retorna apenas os vínculos do professor logado
...
class SchoolViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Se for GESTOR, vê as escolas que ele gerencia
        if user.role.lower() == 'gestor':
            return School.objects.filter(manager=user)
            
        # Se for PROFESSOR, vê as escolas onde ele foi aprovado
